...
Last updated: 15 July 2026 (Session 48 — Domain × group outcomes matrix built, Outcomes & Impact tab default view)
...
- [ ] **HIGH PRIORITY — `entries` RLS blocks mat_admin from reading other schools, breaking the live MAT Dashboard sales demo** — found during Session 47 Phase 0 diagnostic (domain × group matrix investigation). Verified live: the demo `mat_admin` account (`demo@testschool.co.uk`) can only read `entries` rows for its own school (Springwell) — confirmed both via the running app's actual network calls and a direct REST query matching MAT Dashboard's exact `school_id=in.(...)` multi-school fetch, both returning 166 rows (Springwell only) out of the full trust. Confirmed via a direct database connection (bypassing RLS) that the other schools' data genuinely exists and is not empty — Rydell High alone has 150 real `entries` rows (30 with evidence attached), completely invisible to the mat_admin account under current RLS. **This is trust-facing, not just an internal metric** — it means the live MAT Dashboard (used in the sales demo flow, `/demo` → MAT view) currently shows 0% readiness for every school except Springwell, which directly undercuts the "see every school at a glance" pitch this view exists to make. No migration file defines the current `entries`/`evidence_entries` RLS policies at all (predates tracked migration history, like the grp_*/reach_* columns did) — fixing this needs a fresh live `pg_policies` read to see the actual current `USING` clause before writing a `mat_admin`-scoped SELECT policy (likely modeled on the existing `schools_select_scoped` pattern from Session 42's `step14_schools_select_policy.sql`: own school OR same `mat_id` via a `schools` join OR `is_founder`). Needs its own dedicated session — not fixed here, diagnostic only.
