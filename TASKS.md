...
- [x] **Set-password intercept fixed** — invited users now land on /set-password reliably before entering the dashboard. Root cause: if Supabase processes the invite hash before React's useEffect runs, onAuthStateChange fires INITIAL_SESSION (not SIGNED_IN) — our SIGNED_IN-only check missed it. Fix: check both SIGNED_IN and INITIAL_SESSION in onAuthStateChange, plus a belt-and-suspenders check in getSession().then(). SetPasswordPage now shows "Password set successfully" then navigates to /home after 1 second.
