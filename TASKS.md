...
- [ ] **Verify MAT Barriers view renders correctly for all 6 demo schools** — Check Panel 1 school chips display correctly (Springwell navy resolved chips, Rydell amber active chips). Confirm Panel 3 shows SEMH shared pattern (Springwell being_addressed + Rydell active in Wellbeing/Mental Health). Confirm Panel 2 shows Rydell and Sunnydale with unacknowledged gaps. Note: Rydell LAC named person barrier has no provision link — verify it appears in Panel 2 correctly despite missing link.
